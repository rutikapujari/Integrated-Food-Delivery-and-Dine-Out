...
import { User, Envelope, Phone, Lock, ShieldCheck, ArrowLeft } from '../../utils/icons'
...
const registerSchema = z.object({
  ...
  adminRegistrationCode: z.string().min(1, 'Admin registration code is required'),
...
function AdminRegisterPage() {
  ...
  const onRegister = async (data) => {
    ...
    const result = await dispatch(registerUser({ ...userData, role: 'admin' }))
  ...
  const onVerify = async (data) => {
    ...
    if (verifyOTP.fulfilled.match(result)) {
      notify.success('Admin account verified! Sign in to continue.')
        ...
        <div className="bg-slate-800 rounded-xl border border-slate-700 p-8">
          ...
          <form onSubmit={hs1(onRegister)} className="space-y-4">
            ...
            <Input
              ...
              icon={ShieldCheck}
              error={e1.adminRegistrationCode?.message}
              disabled={loading}
              register={reg1('adminRegistrationCode')}
              className="bg-slate-700 border-slate-600 text-white placeholder:text-slate-400 focus:border-primary"
            />
